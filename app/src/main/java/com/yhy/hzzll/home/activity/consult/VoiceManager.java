...
import com.yhy.hzzll.home.activity.consult.utils.CommonDefine;
import com.yhy.hzzll.home.activity.consult.utils.FileUtils;
...
import static com.yhy.hzzll.home.activity.consult.utils.TimeMethod.timeSpanSecond;

public class VoiceManager {
    public ArrayList<File> mRecList = new ArrayList<File>();
    private Activity mActivity = null;
    private View mVRecRoot, mVPlaRoot, mVRecFinish;
    private ImageView mIVRecOperate, mIVPlaOperate, mIVPlaStop;
    private TextView mTVRecText, mTVRecTime, mTVPlaCurrent, mTVPlaTotal,tv_res_status;
    private SeekBar mSBPlayProgress;
    private int mSavedState, mDeviceState = CommonDefine.MEDIA_STATE_UNDEFINE;
    private MediaRecorder mMediaRecorder = null;
    public MediaPlayer mMediaPlayer = null;
    public String mRecTimePrev, mPlaFilePath;
    public long mRecTimeSum = 0;
                ...
                switch (msg.what) {
                    case CommonDefine.MSG_TIME_INTERVAL:
                        if (mDeviceState == CommonDefine.MEDIA_STATE_RECORD_DOING) {
                            ...
                            ts = timeSpanSecond(mRecTimeSum);
                            mTVRecTime.setText(String.format("%02d:%02d:%02d", ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond));
                            ...
                            if (ts.mSpanMinute >= 3){
                                ...
                                ToastUtils.getUtils(mActivity).show("录音时长已达上限");
                            }
                            ...
                            ts = timeSpanSecond(current / 1000);
                            mTVPlaCurrent.setText(String.format("%02d:%02d:%02d", ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond));
    ...
    public VoiceManager(Activity act, String filePath) {
        ...
        tv_res_status = (TextView) mActivity.findViewById(R.id.tv_res_status);
            ...
            mIVRecOperate.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (mDeviceState == CommonDefine.MEDIA_STATE_RECORD_DOING) {
                        ...
                        mMediaRecorder = null;
                        mTVRecText.setText("已暂停");
                        tv_res_status.setText("继续录音");
                        mIVRecOperate.setImageResource(R.drawable.voice_play);
        ...
        if (mSBPlayProgress != null) {
            mSBPlayProgress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                ...
                public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                    ...
                    TimeMethod ts = timeSpanSecond(progress / 1000);
                    mTVPlaCurrent.setText(String.format("%02d:%02d:%02d", ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond));
                }
...
//            long min = TimeMethod.timeSpanSecond(mRecTimeSum).mSpanMinute;
//            long s = TimeMethod.timeSpanSecond(mRecTimeSum).mSpanSecond;
//            s = s + min * 60;

            if (TimeMethod.timeSpanSecond(mRecTimeSum).mSpanSecond< 10 && TimeMethod.timeSpanSecond(mRecTimeSum).mSpanMinute==0) {
                ...
                return;
            }else if (TimeMethod.timeSpanSecond(mRecTimeSum).mSpanMinute == 3&& TimeMethod.timeSpanSecond(mRecTimeSum).mSpanSecond > 1){
                Toast.makeText(mActivity, "时间过长,请重新录音！", Toast.LENGTH_SHORT).show();
                mActivity.finish();

//                voiceCallBack.recFinish();
//                voiceCallBack.voicePath("", 0);
                ...
                if (file != null && file.length() > 0) {
                    ...
                    voiceCallBack.recFinish();
                    voiceCallBack.voicePath(file.getAbsolutePath(), mRecTimeSum);
                }
            }
...
//            mIVPlaOperate.setImageResource(R.drawable.voice_pause);
            ...
            if (prepareMedia(mMediaPlayer, path)) {
                ...
                TimeMethod ts = timeSpanSecond(mMediaPlayer.getDuration() / 1000);
                mTVPlaTotal.setText(String.format("%02d:%02d:%02d", ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond));
     ...
     * @return
     */
    public boolean stopMedia(MediaPlayer mp, boolean release) {
        boolean result = false;
