...
public class ClipView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (clipType == ClipType.CIRCLE) {
            ...
            float a  = (this.getWidth()-2*mHorizontalPadding)*2/3;
            canvas.drawRect(mHorizontalPadding, this.getHeight() / 2 - a,
                    this.getWidth() - mHorizontalPadding, this.getHeight() / 2 + a, paint);
            //绘制白色的矩形边框
            canvas.drawRect(mHorizontalPadding, this.getHeight() / 2 - a,
                    this.getWidth() - mHorizontalPadding, this.getHeight() / 2 + a, borderPaint);
        }
    ...
    public Rect getClipRect() {
        Rect rect = new Rect();
        int  a  = (int)(this.getWidth()-2*mHorizontalPadding)*2/3;
        //宽度的一半 - 圆的半径
        rect.left =(this.getWidth() / 2 - clipRadiusWidth);
        //宽度的一半 + 圆的半径
        rect.right = (this.getWidth() / 2 + clipRadiusWidth);
        //高度的一半 - 圆的半径
        rect.top = (this.getHeight() / 2 - a);
        //高度的一半 + 圆的半径
        rect.bottom = (this.getHeight() / 2 + a);
        return rect;
    }
